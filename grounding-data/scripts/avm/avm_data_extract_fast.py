...
def process_bicep_file(bicep_file_path):
    ...
    """
    # This function's logic remains the same, as it correctly processes
    # one file and returns one dictionary object.
...
def main():
    ...
    # Open the output file once. We will write to it as results come in.
    with open(output_filename, 'w', encoding='utf-8') as f:
        with ProcessPoolExecutor() as executor:
            # The map function returns an iterator. We loop through it as results complete.
            results = executor.map(process_bicep_file, bicep_files_to_process)

            for result in results:
                # Check if the processing was successful
                if result is not None:
                    # Convert the single dictionary object to a JSON string
